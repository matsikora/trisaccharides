...
class martini3(object):
    ...
    def __init__(self):
        ...
        self.sugarinternals = {
            ...
            #             C12 345  C67 NAc C89      C12-C34       C34-C56       C12-C56       C12-2Ac       C34-C12-2Ac C34-C12-2Ac    
            "0SA":  [spl("SQa SNda SP1 SP2  P4"),[(0.276, None),(0.307, None),(0.334,18000),(0.300,15000)],[  ( 90,200) , (120,200)],],
    ...
    def messages(self):
        '''Prints any force-field specific logging messages.'''
        import logging
        logging.info('Note: Disulfide bonds are 0.24 nm constraints, instead of the published (0.39 nm, 5000 kJ/mol) bond.')
...
def sugarmap(sugar, red_end=False):
    ...
    p = CoarseGrained.sugarmapping[sugar.name]
    
    # If sugar is the reducing end of a free glycan, its first bead needs to
    # include the ROH atoms
    if red_end:
        logging.warning("I don't know yet how to include the ROH beads at the reducing end!")
    ...
    q = [[(m, coord, a.index((atom, m, coord))) for atom, m, coord in a if atom in bead] for bead in p]
...
class Sugar(object):
    ...
    def __init__(self, linkline, reslist):  
        # ('ND2',    'ASN',     132,             'C1',    '4YB',     211        )
        atm1name, res1name, res1num, chain1, atm2name, res2name, res2num, chain2 = linkline        
        
        # NOTE: Empty chain ids come as None from pdbLink. 
        self.linkingatom = (atm2name, res2name, res2num, chain2 or "")
        self.link_to     = (atm1name, res1name, res1num, chain1 or "")      # ASN, SER, THR typically, or another sugar, or ROH for a free reducing end     
        self.red_end_carbon_atom = self.linkingatom[0]                      # most often 'C1'; 'C2' for sialic acids, fructose

        # Residue name, number and chain
        self.resname, self.resnum, self.chain = self.linkingatom[1:]
        
        # Name (GlcNAc, Man, Gal, Neu5Ac,...)
        try:
            self.name = glycamCode[self.resname]
        except KeyError:
            logging.warning('Residue name {} was not found in the glycamCode dict.'.format(self.resname))
        ...
        self.longname = '-'.join([self.anomer, self.D_L, self.name[:3]]) + self.ringsize + self.name[3:] 
        
        # Get atom names from Residue instance and the Residue instance itself
        self.residue = None
        for res in reslist:
            if res[0][1:3] == self.linkingatom[1:3] and res[0][3] == chain2:
                self.atoms = [res[i] for i in res]
                self.residue = res
        if not self.residue:
            logging.error('Residue not found that contains sugar {}!'.format(self.linkingatom[1:]))
            sys.exit(1)
        
                        
        # Dictionary of substituents
        self.subst = {self.red_end_carbon_atom: self.link_to[1:]}
...
class Glycan(Chain):
    ...
    def __init__(self, sugar, options=[]):
        ...
        if self.anchorres[0:2] == ('ND2', 'ASN'):
            ...
            self.name = 'Linear free glycan'
        else:
            self.name = 'Special glycan (missing link?)'

    def __iadd__(self, sugar):
    ...
    def __str__(self):
        return ('{} anchored to {}'.format(self.name, self.anchorres))
    ...
    def _seq(self):
        return ''.join(['('+sugar.name+')' for sugar in self.sugars])
    ...
    def cg(self, force=False, com=False):
        ...
        logging.debug('{} has {} sugars'.format(self.name, len(self.sugars)))
        for sugar in self.sugars:
            ...
            # is inferred from the sequence. So this is the best place to raise
            # an error
            ## TODO: Implement test and treatment of the reducing end
            try:
                ...
                if ci.multiscale:
                    for r in ci.residues:
                        for name, resn, resi, chain, x, y, z in r:
                            ...
                            atid += 1
                coarseGrained = ci.cg(force=True, com=True)
